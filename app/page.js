...
export default function Home() {
  ...
  const copyToClipboard = async (text, label = "Text") => {
    try {
      ...
      actions.setCopiedText(label);
      setTimeout(() => actions.setCopiedText(""), 2000);
    } catch (err) {
  ...
  const handleImageUpload = (urls) => {
    actions.addImageUrls(urls);
  };

  const handleAnalyze = async () => {
    if (!imageUrls || imageUrls.length === 0) {
      actions.setError("Please upload report images first.");
      return;
    }
    ...
    try {
      ...
      actions.setAnalysis(aiResult);
      actions.refreshHistory(); // Refresh history after successful analysis
    } catch (err) {
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-white">
        <ResponsiveHeader onChatOpen={() => actions.setChatOpen(true)} />
          ...
          <div className="bg-white border border-gray-200 p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl shadow-xl space-y-4 sm:space-y-6 lg:space-y-8">
            ...
            {imageUrls && imageUrls.length > 0 && (
              <ResponsiveImageGallery
                ...
                onClearAll={actions.clearImageUrls}
              />
            )}
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 sm:p-6 space-y-4 sm:space-y-6">
              <div>
                ...
                <select
                  ...
                  name="testType"
                  value={testType}
                  onChange={(e) => actions.setTestType(e.target.value)}
              ...
              <AnalysisResults
                ...
                onCopyAll={() => {
                  const fullAnalysis = analysis.reports
                    ...
                    .join("\n\n");
                  copyToClipboard(fullAnalysis, "Complete Analysis Results");
                }}
                onChatOpen={() => actions.setChatOpen(true)}
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (
                                  ...
                                  reportContent += `LABORATORY PARAMETERS:\n`;
                                  report.analysis.parameters.forEach(
                                    (param) => {
                                      reportContent += `• ${param.name}: ${param.value}`;
                                      if (param.referenceRange) {
                                        reportContent += ` (Reference: ${param.referenceRange})`;
                      ...
                      {report.success ? (
                        <div className="space-y-4 sm:space-y-6">
                          ...
                          {report.analysis.clinicalInsights && (
                            <div className="space-y-3">
                              <h5 className="text-sm sm:text-base font-semibold text-black mb-3 flex items-center">
                                <span className="mr-2">🔍</span> Clinical
                                Insights
                              </h5>
                          ...
                          {report.analysis.metadata && (
                            <div className="bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-lg p-3 sm:p-4">
                              <h5 className="text-sm font-semibold text-gray-800 mb-3 flex items-center">
                                <span className="mr-2">ℹ️</span> Analysis
                                Details
                              </h5>
